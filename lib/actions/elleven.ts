...
export async function sincronizarVendedoresElleven(decisoes: DecisaoVendedorElleven[]) {
  ...
  await registrarAuditoria({
    acao: "EDITAR",
    modulo: "ELLEVEN",
    detalhes: `Sincronizou vendedores do Elleven (${criados} criados, ${renomeados} renomeados, ${cidadesDefinidas} cidades definidas)`,
    metadata: { criados, renomeados, cidadesDefinidas },
  });
...
// ---------- Importação Manual de Cancelamentos (Cancelamentos por Mês - Claude) ----------

export async function importarCancelamentosElleven(periodo: string, csvText: string) {
  await requireAdmin();

  const cancelamentos = parseCsvCancelamentos(csvText);
  if (cancelamentos.length === 0) {
    return { ok: false as const, error: "Nenhum cancelamento válido encontrado no arquivo." };
  }
  ...
  for (const c of cancelamentos) {
    if (c.contrato) {
      await prisma.contratoAtivacaoElleven.updateMany({
        where: { contrato: c.contrato.trim() },
  ...
  await registrarAuditoria({
    acao: "IMPORTAR",
    modulo: "ELLEVEN",
    detalhes: `Importou ${atualizados} cancelamentos do Elleven para o período ${periodo}`,
    metadata: { periodo, cancelamentos: atualizados },
  });

  revalidatePath("/importar/elleven");
  revalidatePath("/fechamento");
  return { ok: true as const, processados: atualizados };
}
