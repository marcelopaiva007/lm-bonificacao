...
export async function createRegraBonificacao(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await registrarAuditoria({
    acao: "CRIAR",
    modulo: "REGRAS",
    detalhes: `Cadastrou nova regra de bonificação para o cargo ${parsed.data.cargo} (vigência: ${parsed.data.vigenciaInicio})`,
    metadata: { cargo: parsed.data.cargo, vigenciaInicio: parsed.data.vigenciaInicio },
  });
