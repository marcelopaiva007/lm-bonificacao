...
export async function confirmarImportacao(input: z.infer<typeof importSchema>): Promise<ActionResult> {
  ...
  await registrarAuditoria({
    acao: "IMPORTAR",
    modulo: "IMPORTACAO",
    detalhes: `Importou ${parsed.data.linhas.length} lançamentos de vendas do arquivo "${parsed.data.arquivoNome}" (período ${parsed.data.periodo})`,
    metadata: { periodo: parsed.data.periodo, arquivoNome: parsed.data.arquivoNome, linhas: parsed.data.linhas.length },
  });
