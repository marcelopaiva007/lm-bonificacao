...
export async function createLancamento(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  const l = await prisma.lancamentoVenda.create({ data: { ...parsed.data, origem: "MANUAL" } });
  await recalcularFechamento(parsed.data.periodo);

  await registrarAuditoria({
    acao: "CRIAR",
    modulo: "LANCAMENTOS",
    detalhes: `Criou lançamento manual de venda para o período ${parsed.data.periodo} (Qtd: ${parsed.data.quantidade})`,
    metadata: { lancamentoId: l.id, periodo: parsed.data.periodo, funcionarioId: parsed.data.funcionarioId },
  });
...
export async function updateLancamento(id: string, _prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await registrarAuditoria({
    acao: "EDITAR",
    modulo: "LANCAMENTOS",
    detalhes: `Atualizou lançamento de venda (ID: ${id}, período: ${parsed.data.periodo})`,
    metadata: { lancamentoId: id, periodo: parsed.data.periodo, funcionarioId: parsed.data.funcionarioId },
  });
...
export async function deleteLancamento(id: string): Promise<ActionResult> {
  ...
  await registrarAuditoria({
    acao: "EXCLUIR",
    modulo: "LANCAMENTOS",
    detalhes: `Excluiu lançamento de venda (ID: ${id}, período: ${lancamento.periodo})`,
    metadata: { lancamentoId: id, periodo: lancamento.periodo, funcionarioId: lancamento.funcionarioId },
  });
...
export async function createAjuste(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  const ajuste = await prisma.ajuste.create({ data: { ...parsed.data, fechamentoId: fechamento.id } });
  await recalcularFechamento(parsed.data.periodo);

  await registrarAuditoria({
    acao: "CRIAR",
    modulo: "FECHAMENTO",
    detalhes: `Criou ajuste de R$ ${parsed.data.valor} no mês ${parsed.data.periodo}: "${parsed.data.descricao}"`,
    metadata: { ajusteId: ajuste.id, periodo: parsed.data.periodo, valor: parsed.data.valor },
  });
...
export async function deleteAjuste(id: string): Promise<ActionResult> {
  ...
  await registrarAuditoria({
    acao: "EXCLUIR",
    modulo: "FECHAMENTO",
    detalhes: `Excluiu ajuste de R$ ${ajuste.valor} no mês ${ajuste.periodo}: "${ajuste.descricao}"`,
    metadata: { ajusteId: id, periodo: ajuste.periodo, valor: ajuste.valor },
  });
