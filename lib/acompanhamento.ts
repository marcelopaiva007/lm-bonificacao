...
export function montarMensagem(
  ...
  // Se a pessoa não possui nenhuma venda no mês, exibe mensagem informativa apropriada
  // em vez de parabenizar por metas desbloqueadas.
  if (agregado && agregado.quantidade === 0) {
    return `Olá, ${primeiroNome}! Você ainda não possui vendas registradas neste mês. Realize suas primeiras ativações para acompanhar seu progresso de metas aqui. 💡`;
  }
...
export function calcularAcompanhamento(params: {
  ...
  return {
    ...
    supervisor,
    bonificacaoAtual,
    mensagem: montarMensagem(nome, metas, supervisor, agregado),
  };
}
