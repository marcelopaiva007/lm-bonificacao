...
// Parser do relatório "Cancelamentos por Mês - Claude" do Exportador de Dados do Elleven
export function parseCsvCancelamentos(csvText: string): LinhaCancelamento[] {
  const linhas = csvText.split(/\r?\n/).filter((l) => l.trim().length > 0);
  if (linhas.length <= 1) return [];

  const separador = linhas[0].includes(";") ? ";" : ",";
  const cabecalhos = linhas[0].split(separador).map((c) => normalizarTexto(c.replace(/^"|"$/g, "")));

  const idxContrato = cabecalhos.findIndex((c) => c.includes("contrato"));
  const idxVendedor = cabecalhos.findIndex((c) => c.includes("vendedor"));
  const idxCliente = cabecalhos.findIndex((c) => c.includes("cliente") || c.includes("nome"));
  const idxData = cabecalhos.findIndex((c) => c.includes("cancelamento") || c.includes("data"));
  const idxMotivo = cabecalhos.findIndex((c) => c.includes("motivo") || c.includes("status"));
  const idxCidade = cabecalhos.findIndex((c) => c.includes("cidade") || c.includes("local"));

  const resultado: LinhaCancelamento[] = [];

  for (let i = 1; i < linhas.length; i++) {
    const colunas = linhas[i].split(separador).map((c) => c.replace(/^"|"$/g, "").trim());
    if (colunas.length < 2) continue;

    const contrato = idxContrato >= 0 ? colunas[idxContrato] : colunas[0];
    if (!contrato) continue;

    resultado.push({
      contrato,
      vendedor: idxVendedor >= 0 ? colunas[idxVendedor] : "",
      cliente: idxCliente >= 0 ? colunas[idxCliente] : "",
      dataCancelamento: idxData >= 0 ? colunas[idxData] : null,
      motivo: idxMotivo >= 0 ? colunas[idxMotivo] : null,
      cidade: idxCidade >= 0 ? colunas[idxCidade] : null,
    });
  }
