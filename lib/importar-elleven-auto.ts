...
export async function importarLancamentosEllevenAuto(
  ...
  for (const lin of cancelamentosLinhas) {
    const dados = lin.dados as Record<string, string>;
    const numContrato = lin.chave || dados?.contrato || dados?.Contrato;
    if (numContrato) contratosCanceladosSet.add(numContrato.trim());
  }
  ...
  // Marca status como Cancelado para contratos que constem no relatório de cancelamentos do mês
  for (const c of doPeriodo) {
    if (c.contrato && contratosCanceladosSet.has(c.contrato.trim())) {
